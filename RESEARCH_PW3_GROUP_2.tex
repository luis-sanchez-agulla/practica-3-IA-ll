...
categorías de noticias (World, Sports, Business y Sci/Tech) a partir del corpus news\_dataset.csv.
El trabajo se articula en dos ejercicios consecutivos: el primero aborda la generación de
\textit{word embeddings} propios mediante el algoritmo Skip-Gram con \textit{Negative Sampling},
explorando el efecto de la dimensionalidad del vector y el tamaño de la ventana de contexto
sobre la calidad de las representaciones; el segundo utiliza dichos embeddings como capa
de entrada de clasificadores basados en arquitecturas recurrentes LSTM y GRU, evaluando
doce configuraciones de hiperparámetros. El mejor modelo obtenido, \texttt{LSTM\_d64\_1L\_lr001},
alcanza una \textit{val\_accuracy} del 90,49\,\%, lo que demuestra que embeddings entrenados
desde cero sobre un corpus temáticamente coherente son suficientes para obtener un rendimiento
